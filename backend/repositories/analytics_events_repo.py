...
pooler hiccup while emitting an activity-feed event doesn't drop the event
silently. Reads are best-effort with a single retry.
"""
...
@with_retry(name="events_repo.insert_event", attempts=3, fallback=False)
...
@with_retry(name="events_repo.recent", attempts=2, fallback=[])
